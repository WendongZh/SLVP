...
def test_wrapper(model,test_model):
    model.load(args.pretrained_model)
    test_input_handle = datasets_factory.data_provider(
        args.dataset_name, args.train_data_paths, args.valid_data_paths, args.batch_size, args.img_width,
        seq_length=args.total_length, is_training=False)
    trainer.test(model, test_model,test_input_handle, args, 'test_result')

"""
Record the MSE and SSIM infomation to display the trainning  
"""
def display_final(log, log_sample, img_suffix = '', file_suffix = 'train_log.pkl', given_save_dir = None):
    ...
    file_name_sample = open(os.path.join(save_dir, 'test_log_sample.pkl'), "wb")
    pkl.dump(log_sample, file_name_sample)
...
def train_all_kth_actions(model, pre_model=None,test_model=None):
    ...
    for i, item in enumerate(categories):
        ...
        train_log = CPL_train_wrapper(args, model,test_model, pre_model,writer = writer,
                    save_prefix=item + '-', kth_specific_category = None,category=i)
        ...
        test_log = test_all_kth_actions(model, test_model,False)
    ...
    # Train log save as the train_log_list.pkl in the direction train_all_mnist_digits_log_save_dir
    ...
    # Test log save as the test_log_list.pkl in the direction train_all_mnist_digits_log_save_dir
    log_file_name = "test_log_list.pkl"
    f = open(os.path.join(args.train_all_kth_actions_log_save_dir, log_file_name), "wb")
    pkl.dump(test_log_list, f)
    print("test_log_list : ", test_log_list, end="\n\n\n")

def test_all_kth_actions(model,test_model, is_loading_pretrain = True):
    if is_loading_pretrain:
        model.load(args.pretrained_model)


    categories = ['boxing', 'handclapping', 'handwaving', 'walking', 'jogging', 'running']
    # categories = ['boxing', 'handclapping', 'running']
    # categories = ['boxing']
    ...
    for i, item in enumerate(categories):
        ...
        record = trainer.test(model, test_model,test_input_handle, args, 'test_result' + str(item),i)
...
def test_all_kth_actions_final_lpips(model,test_model, is_loading_pretrain = True):
    if is_loading_pretrain:
        model.load(args.pretrained_model)
        test_model.load(args.pretrained_model)
    ...
    for i, item in enumerate(categories):
        ...
        test_input_handle = datasets_factory.data_provider(
            args.dataset_name, args.train_data_paths, args.valid_data_paths, args.batch_size, args.img_width,
            seq_length=args.total_length, is_training=False, kth_specific_category=[item])
        record = trainer.test_final_lpips(model, test_model,test_input_handle, args, 'test_result' + str(item),i)

        test_log["iter"].append(categories[i])
        test_log["PSNR"].append(record["Final avg_psnr_all"])
        test_log["SSIM"].append(record["Final avg_ssim_all"])
        test_log["LPIPS"].append(record["Final avg_lpips_all"])
        test_log_sample[categories[i]]=record['Final mean_psnr_all']


    test_log["iter"].append("Mean")
    test_log["PSNR"].append(np.mean(test_log["PSNR"], 0))
    test_log["SSIM"].append(np.mean(test_log["SSIM"], 0))
    test_log["LPIPS"].append(np.mean(test_log["LPIPS"], 0))
    print(test_log)
    return test_log, test_log_sample
...
if args.train_all_mnist_digits_log_save_dir != None:
    if os.path.exists(args.train_all_mnist_digits_log_save_dir):
        shutil.rmtree(args.train_all_mnist_digits_log_save_dir)
    os.makedirs(args.train_all_mnist_digits_log_save_dir)
...
if args.dataset_name == "action":
    if args.is_training:
        if args.is_multi_training:
            train_all_kth_actions(model, pre_model,test_model)
        else:
            print('joint training')
            writer = SummaryWriter(args.tensorboard_dir)
            train_log = train_wrapper(args, model,test_model, writer=writer)
            display(train_log)
            writer.export_scalars_to_json(os.path.join(args.tensorboard_dir, "all_datas.json"))
            writer.close()
    else:
        ...
        if args.is_multi_testing:
            test_log, test_log_sample = test_all_kth_actions_final_lpips(model,test_model)
            display_final(test_log, test_log_sample, file_suffix="mutil_testing_log.pkl",
                    given_save_dir=args.test_kth_actions_save_path)
        else:
            test_wrapper(model,test_model)