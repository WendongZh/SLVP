...
def data_provider(dataset_name, train_data_paths, valid_data_paths, batch_size,
                  img_width, seq_length, is_training=True, kth_specific_category=None, no_shuffle=False, label_file=None):
    ...
    if dataset_name == 'action':
        ...
        if is_training:
            train_input_handle = input_handle.get_train_input_handle(specific_category = kth_specific_category, label_file=label_file)
            if no_shuffle:
