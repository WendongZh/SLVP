...
class InputHandle:
    def __init__(self, datas, indices,category, input_param, mode=None, category_kmeans=None):
        ...
        self.index_out = np.arange(len(self.indices))
        self.index_inner = []
        self.category_kmeans = category_kmeans
        print(np.shape(category_kmeans))
    ...
    def begin(self, do_shuffle=True):
        ...
        if do_shuffle:
            ...
            random.shuffle(self.index_out)
        else:
            self.index_out = np.arange(len(self.indices))
        self.current_position = 0
        # self.current_batch_indices = self.indices[self.current_position:self.current_position + self.minibatch_size]
        self.index_inner = self.index_out[self.current_position:self.current_position + self.minibatch_size]
        self.current_batch_indices = self.indices[self.index_inner]
    ...
    def get_batch(self):
        ...
        input_category_kmeans = np.zeros(self.minibatch_size)
        ...
        if self.category_kmeans is not None:
            input_category_kmeans = self.category_kmeans[self.index_inner][:, 0]

        return input_batch, input_category, input_category_kmeans
...
class DataProcess:
    def __init__(self, input_param):
        ...
        tmp = 'Right_kth_kmeans6_label_allkth_64nonorm.npy'
        print(tmp)
        self.train_kmeans_label = np.load(tmp)

        self.input_param = input_param
        self.seq_len = input_param['seq_length']

    # Specific_category is for Continual learning for different categories
    def load_data(self, paths, mode='train', specific_category = None, label_file=None):
        '''
        ...
        if label_file is not None:
            print('current label file is %s' % label_file)
            self.train_kmeans_label = np.load(label_file)
        ...
        for i,c_dir in enumerate(c_dir_list): # handwaving
            ...
            ############################## Important add .sort() to match the kmeans labels
            p_c_dir_list.sort()
        ...
        return data, indices, frames_specific_category, self.train_kmeans_label

    def get_train_input_handle(self, specific_category = None, label_file=None):
        train_data, train_indices,train_category, kmeans_category = self.load_data(self.paths, mode='train', specific_category = specific_category, label_file=label_file)
        return InputHandle(train_data, train_indices,train_category, self.input_param, mode='train', category_kmeans=kmeans_category)

    def get_test_input_handle(self, specific_category = None):
        test_data, test_indices,test_category, _ = self.load_data(self.paths, mode='test', specific_category = specific_category)
        return InputHandle(test_data, test_indices, test_category,self.input_param, mode='test')
