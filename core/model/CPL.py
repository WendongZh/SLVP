...
class RNN(nn.Module):
    ...
    def forward(self, frames_tensor,mask_true,category=None,is_train=True,is_replay=False, final_generate=False):
        ...
        c_test = []
        h_test = []
        # y_target=torch.ones(self.configs.batch_size,device=self.configs.device,dtype=torch.long)*category
        # Continual training
        if not is_train:
            if final_generate:
                category = category.long()
                y_gt = torch.nn.functional.one_hot(category, num_classes=self.configs.num_classes)
                y_gt = y_gt.float()
            else:
                y_gt=torch.eye(self.configs.num_classes,device=self.configs.device)[category]
                
                current_batch = batch // self.configs.num_samples
                y_gt=y_gt.repeat(current_batch,1)
        else:
            # joint training
            category = category.long()
            y_gt = torch.nn.functional.one_hot(category, num_classes=self.configs.num_classes)
        ...
        if is_train:
            ...
            y0=1/self.configs.num_classes
            y_uni=[y0,y0,y0,y0,y0,y0]
            y_uni=torch.tensor(y_uni,device=self.configs.device).repeat(self.configs.batch_size,1)
